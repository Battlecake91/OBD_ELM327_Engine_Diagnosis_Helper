...
def test_opel_probe_uses_fast_init_target_11_and_reads_identification():
    elm = FakeELM(
        {
            "ATFI": ["OK\r>"],
            ...
            "1A80": [
                "80 F1 11 0A 5A 80 44 45 4C 20 58 31 36 58 45 4C 00\r>"
            ],
    ...
    assert "ATAL" in elm.commands
    assert "ATSH8111F1" in elm.commands
    assert "ATWM8111F13E" in elm.commands
    assert "ATFI" in elm.commands
    assert "81" in elm.commands
    assert "1A80" in elm.commands
    ...
    assert "POSITIVE 0x5A 0x80" in report
    assert "DEL X16XEL" in report
    assert any("> 1A80" in entry for entry in log)
...
def test_opel_probe_falls_back_to_target_10_after_no_data_on_11():
    ...
    assert elm.commands.count("1A80") == 1
    assert "target ECU 0x10" in report
    assert "KWP StartCommunication: SUCCESS" in report


def test_opel_probe_does_not_identify_when_fast_init_fails():
    elm = FakeELM(
        {
            "ATFI": ["?\r>", "?\r>"],
    ...
    assert elm.commands.count("ATFI") == 2
    assert "1A80" not in elm.commands
