...
# Keep the historical settings token so existing installations retain their selection.
# The token name is intentionally not exposed to users; the actual protocol is
# verified ISO 14230-4 KWP2000 Fast Init.
OPEL_PROTOCOL_TOKEN = "OPEL_KW82_9600"
OPEL_PROTOCOL_LABEL = "Opel Astra G X16XEL / Multec-H"
...
class OpelKwpWorker(core.OBDWorker):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.elm.open()
            identity = initialize_opel_adapter(self.elm, self.raw_log.emit)
            success, report = probe_opel_engine(self.elm, self.raw_log.emit)
